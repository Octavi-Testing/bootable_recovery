...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    ...
    if (awaken_logo_ && back_icon_) {
      auto logo_width = gr_get_width(awaken_logo_.get());
      auto logo_height = gr_get_height(awaken_logo_.get());
      auto centered_x = ScreenWidth() / 2 - logo_width / 2;
      DrawSurface(awaken_logo_.get(), 0, 0, logo_width, logo_height, centered_x, y);
      y += logo_height;
...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
  ...
  int menu_rows = (ScreenHeight() - margin_height_*2 - gr_get_height(awaken_logo_.get()))
